...
With **Input type** (`input_type`) set to `archive` (the default), select a
`.zip`, `.tar.gz`, or `.tgz` archive containing at least
...
Set **Input type** (`input_type`) to `list` and select a text file such as
`all_list.txt` as **Input** (`input`). Each nonempty line contains one FASTA
...
Relative entries are resolved against **Base path for relative FASTA list
entries** (`list_base`). If it is empty, they are resolved against the
directory containing the list. Full `s3://BUCKET/key` paths are used unchanged.
...
If the actual S3 bucket name is `pangrowth`, use:

| Parameter | Value |
| --- | --- |
| `input` | `s3://pangrowth/yeast/cerevisiae-n81/all_list.txt` |
| `input_type` | `list` |
| `list_base` | `s3://pangrowth/` |
| `outdir` | `s3://pangrowth/results/cerevisiae-n81/` |
...
For a local run from the repository root with this same list, the base is
`data`:

```bash
nextflow run . --input data/yeast/cerevisiae-n81/all_list.txt \
    --input_type list --list_base data --outdir results/cerevisiae-n81
```
